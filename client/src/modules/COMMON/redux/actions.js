  ...
  return async (dispatch) => {
    ...
    try {
      const main = await axios.get(
        `${
          ["schedule, projections"].includes(item) ? "/lineups" : ""
        }/main/${item}`
      );
      ...
      const data =
        item !== "projections"
          ? main.data[0]
          : main.data[0].reduce((result, item) => {
  ...
  return async (dispatch) => {
    ...
    try {
      ...
      if (!user.data?.error) {
        ...
        dispatch({
          type: "SET_STATE_COMMON",
          payload: { state: user.data.state },
        });

        const week_lc =
          user.data.state.season_type === "regular"
            ? user.data.state.leg
  ...
  return async (dispatch) => {
    ...
    try {
      const response = await fetch(
        `/league/find?user_id=${encodeURIComponent(user_id)}`,
        {
          method: "GET",
          headers: {
